...
public class FunctionRegistry {
    ...
    public static class ConstructorMeta {
        ...
        private final Constructor<? extends JMFunction> constructor;
    }
    ...
    /**
     * Discover and register {@link JMFunction} implementations that are annotated with {@link JMFunctionImplementation}.
     *
    ...
    public synchronized void discover(List<String> packages) {
        ...
        final Set<Class<? extends JMFunction>> classes = reflections.getSubTypesOf(JMFunction.class);
        classes
                .stream()
                .filter(type -> type.getAnnotation(JMFunctionImplementation.class) != null)
                .forEach(this::register);
        discoveredAlready = true;
    }

    /**
     * Register a {@link JMFunction} implementation. Needs to be annotated with {@link JMFunctionImplementation}.
     *
     * @param clazz Function class.
     */
    public void register(Class<? extends JMFunction> clazz) {
        final JMFunctionImplementation annotation = clazz.getAnnotation(JMFunctionImplementation.class);
    ...
    /**
     * Find a {@link JMFunction} implementation by name.
     *
    ...
    private static List<ConstructorMeta> constructors(Class<? extends JMFunction> type) {
        final Constructor<? extends JMFunction>[] declaredConstructors
                = (Constructor<? extends JMFunction>[]) type.getDeclaredConstructors();
        JMFunctionImplementation annotation = type.getAnnotation(JMFunctionImplementation.class);
                ...
                .map(declaredConstructor -> {
                    ...
                    final List<Class<?>> paramTypes = Arrays.stream(declaredParamTypes)
                            .filter(parameterType -> parameterType.isArray()
                                    ...
                                    .isAssignableFrom(JMTreeNode.class)
                                    : parameterType.isAssignableFrom(JMTreeNode.class))
                            .collect(Collectors.toList());
                    Preconditions.checkArgument(
